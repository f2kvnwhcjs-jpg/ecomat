...
const COPT = ['', 'CH-001', 'CH-002', 'CH-003', 'CH-004', 'CH-005'];
const photoStore = {};
...
const NAV = {
  ...
  merch: [
    ...
    { id: 'm-wbl', ic: 'ti-file-text',            l: 'Накладные' },
  ],
};
const WIDE = ['v-dev', 'v-fin', 'd-dev', 'd-fin', 'm-dev', 'm-fin'];

let role = 'vendor', pg = 'v-d';
let exDist = null, exMer = null, exS = null, exD = null;
let vfF = '2025-05-01', vfT = '2025-05-16';
...
function sR(r) {
  role = r;
  ['vendor', 'dist', 'merch'].forEach((x, i) => document.querySelectorAll('.rt button')[i].classList.toggle('active', x === r));
  pg = NAV[r][0].id; exDist = null; exMer = null; exS = null; exD = null;
  renderSB(); render();
}
function go(p) { pg = p; exD = null; renderSB(); render(); }
...
function render() {
  setW(WIDE.includes(pg));
  const P = { 'v-d': pVDist, 'v-dev': pVDev, 'v-fin': pVFin, 'd-dev': pDDev, 'd-mer': pDMer, 'd-ass': pDAss, 'd-adv': pDAdv, 'd-stm': pDStm, 'd-fin': pDFin, 'm-dev': pMDev, 'm-stk': pMStk, 'm-fin': pMFin, 'm-wbl': pMWbl };
  document.getElementById('main').innerHTML = (P[pg] || pEmpty)();
}
...
/* Readonly channel rows for modal */
function roRows(d) {
  return d.ch.map((ch, i) => {
    const p = ch.l > 0 ? Math.min(Math.round(ch.l / CAP * 100), 100) : 0, col = bc(ch.l), e = ch.l === 0, cl = !ch.a ? 'inact' : e ? 'emp' : 'act', lb = !ch.a ? 'Неактивен' : e ? 'Пустой' : 'Активен';
    return `<div class="rr"><span style="font-size:11px;color:var(--color-text-tertiary);width:52px;flex-shrink:0">К ${i + 1}</span><span style="font-family:var(--font-mono);font-size:11px;width:56px;flex-shrink:0;color:${ch.c ? 'var(--color-text-primary)' : 'var(--color-text-tertiary)'}">${ch.c || '—'}</span><div style="flex:1;height:5px;background:var(--color-border-tertiary);border-radius:3px;overflow:hidden"><div style="height:100%;border-radius:3px;width:${p}%;background:${ch.a && !e ? col : 'var(--color-border-tertiary)'}"></div></div><span style="font-size:11px;font-weight:500;width:54px;text-align:right;color:${ch.a && !e ? col : 'var(--color-text-tertiary)'}">${ch.l > 0 ? ch.l.toFixed(1) + ' л' : '—'}</span><span class="cst ${cl}" style="margin-left:4px;flex-shrink:0">${lb}</span></div>`;
  }).join('');
}
    ...
    `<div style="flex:1;overflow-x:auto;overflow-y:auto"><table class="w">
      ...
      <tbody>${list.map(d => {
        ...
        return `<tr>
          ...
          <td>${isNew ? '<span class="badge bnew"><i class="ti ti-sparkles" style="font-size:10px"></i> Новый</span>' : `<span class="badge ${d.st === 'online' ? 'bon' : 'boff'}"><span class="dot ${d.st === 'online' ? 'dg' : 'dr'}"></span>${d.st === 'online' ? 'Online' : 'Offline'}</span>`}</td>
...
function pVFin() {
  const list = txns.filter(t => { if (vfF && t.date < vfF) return false; if (vfT && t.date > vfT) return false; return true; });
  const tA = list.reduce((a, t) => a + t.amt, 0), tV = list.reduce((a, t) => a + t.vol, 0), tB = list.reduce((a, t) => a + t.bal, 0);
  return hdr('Финансы', `<button class="btn bp" onclick="csvX([['ID','Дата','Дистрибьютор','Мерчант','Устройство','Химия','Объём','Тара','Сумма','Остаток'],...txns.map(t=>[t.id,t.date,t.dist,t.mer,t.dev,t.c,t.vol,t.tara,t.amt,t.bal])],'vendor_fin.csv')"><i class="ti ti-download" style="font-size:12px"></i> Экспорт CSV</button>`) +
    ftb(frow(`<span style="font-size:11px;color:var(--color-text-tertiary);font-weight:500">Период:</span><input type="date" class="tdate" value="${vfF}" onchange="vfF=this.value;render()"><span style="font-size:11px;color:var(--color-text-tertiary)">—</span><input type="date" class="tdate" value="${vfT}" onchange="vfT=this.value;render()"><div style="margin-left:auto">${stats([['Транзакций', list.length], ['Оборот', tA.toLocaleString('ru-KZ') + ' ₸'], ['Объём', tV.toLocaleString('ru-KZ') + ' мл'], ['Нереализовано', tB.toLocaleString('ru-KZ') + ' ₸', tB > 0 ? '#A32D2D' : undefined]])}</div>`)) +
    `<div style="flex:1;overflow-x:auto;overflow-y:auto"><table class="w">
...
function pDMer() {
  const withStm = mercs.filter(m => m.stm).length;
  const frz = mercs.filter(m => m.st === 'frozen').length;
  return `<div class="pad">
  <div class="ph"><h2>Мерчанты</h2></div>
  <div class="sg s5">
  ...
  <div class="card">
    <div class="ch"><span class="ct">Мерчанты</span><button class="btn bp" onclick="openAddMer()"><i class="ti ti-plus"></i> Добавить</button></div>
    <div class="tw"><table class="s">
...
/* Ассортимент — без столбца фото */
function pDAss() {
  return `<div class="pad">
  <div class="ph"><h2>Ассортимент</h2></div>
  <div class="card">
    <div class="ch"><span class="ct">Химии</span><button class="btn bp" onclick="openAddChem()"><i class="ti ti-plus"></i> Добавить</button></div>
    <div class="tw"><table class="s">
      <colgroup><col style="width:12%"><col style="width:22%"><col style="width:14%"><col style="width:11%"><col style="width:24%"><col style="width:10%"><col style="width:7%"></colgroup>
      <thead><tr><th>ID</th><th>Наименование</th><th>Цена /100мл</th><th>Калибровка</th><th>Комментарий</th><th>Изменён</th><th></th></tr></thead>
      <tbody>${chems.map(c => `<tr>
        <td style="font-family:var(--font-mono);font-size:11px;font-weight:500">${c.id}</td>
        <td style="font-weight:500">${c.name}</td>
        <td>${c.price} ₸</td>
        <td style="color:var(--color-text-secondary)">${c.cal}</td>
        <td style="color:var(--color-text-secondary);font-size:11px">${c.comment}</td>
        <td style="color:var(--color-text-secondary);font-size:11px">${fmtDate(c.updatedAt)}</td>
        <td><button class="btn" onclick="openEditChem('${c.id}')"><i class="ti ti-edit" style="font-size:11px"></i></button></td>
      </tr>`).join('')}</tbody>
    </table></div>
  </div></div>`;
}
...
function pDAdv() {
  return `<div class="pad">
  <div class="ph"><h2>Реклама</h2></div>
  <div class="card">
    <div class="ch"><span class="ct">Макеты</span><button class="btn bp" onclick="openAddAd()"><i class="ti ti-plus"></i> Добавить</button></div>
    <div class="tw"><table class="s">
      <colgroup><col style="width:13%"><col style="width:28%"><col style="width:32%"><col style="width:12%"><col style="width:7%"></colgroup>
      <thead><tr><th>ID</th><th>Название</th><th>Комментарий</th><th>Изменён</th><th></th></tr></thead>
      <tbody>${adsList.map(a => `<tr>
        <td style="font-family:var(--font-mono);font-size:11px">${a.id}</td>
        <td style="font-weight:500">${a.name}</td>
        <td style="color:var(--color-text-secondary);font-size:11px">${a.comment}</td>
        <td style="color:var(--color-text-secondary);font-size:11px">${fmtDate(a.updatedAt)}</td>
        <td><button class="btn" onclick="openEditAd('${a.id}')"><i class="ti ti-edit" style="font-size:11px"></i></button></td>
      </tr>`).join('')}</tbody>
    </table></div>
  </div></div>`;
}

/* СТМ — без мини-дашборда */
function pDStm() {
  return `<div class="pad">
  <div class="ph"><h2>СТМ</h2><button class="btn bp" onclick="openShip()"><i class="ti ti-truck"></i> Отгрузка</button></div>
  ${stmM.map(m => {
    const open = exS === m.id;
    return `<div class="mc" onclick="exS=exS===${m.id}?null:${m.id};render()">
      <div style="display:flex;align-items:center;justify-content:space-between">
        ...
        <div style="font-size:11px;color:var(--color-text-secondary);margin-top:2px">${m.stocks.length} позиций · ${m.hist.length} отгрузок</div></div>
      ...
      <div class="md${open ? ' open' : ''}">
        ...
        ${m.stocks.map(s => `<div class="cr">
          <span style="font-family:var(--font-mono);font-size:11px;color:var(--color-text-secondary);width:56px;flex-shrink:0">${s.c}</span>
          <span style="font-size:12px;font-weight:500;flex:1">${CN[s.c] || s.c}</span>
          <div style="width:80px;height:5px;background:var(--color-border-tertiary);border-radius:3px;overflow:hidden;flex-shrink:0"><div style="height:100%;border-radius:3px;width:${Math.min(s.r, 100)}%;background:${s.r < 20 ? '#E24B4A' : s.r < 50 ? '#EF9F27' : '#185FA5'}"></div></div>
          <span style="font-size:11px;font-weight:500;width:46px;text-align:right;color:${s.r < 20 ? '#A32D2D' : 'var(--color-text-primary)'}">${s.r} л</span>
          ${s.r < 20 ? '<span class="badge bwa" style="margin-left:3px">Мало</span>' : ''}
        </div>`).join('')}
        ...
        <tbody>${m.hist.map(h => `<tr>
          <td style="color:var(--color-text-secondary)">${h.date}</td>
          <td><span style="font-family:var(--font-mono);font-size:10px;color:var(--color-text-secondary)">${h.c}</span> ${CN[h.c] || h.c}</td>
          <td style="font-weight:500">${h.v} л</td>
          <td style="font-family:var(--font-mono);font-size:10px;color:var(--color-text-secondary)">${h.wb}</td>
          <td><button class="btn" style="padding:2px 6px" onclick="event.stopPropagation();alert('Печать ${h.wb}')"><i class="ti ti-printer" style="font-size:11px"></i></button></td>
        </tr>`).join('')}</tbody></table>
      </div>
    </div>`;
  }).join('')}
...
function pMStk() {
  ...
  const rows = mStock.map(e => {
    ...
    return `<div class="stk-row">
      <span style="font-family:var(--font-mono);font-size:10px;color:var(--color-text-tertiary);width:54px;flex-shrink:0">${e.id}</span>
      <div style="flex:1">
        <div style="display:flex;align-items:center;gap:7px;margin-bottom:5px"><span style="font-size:13px;font-weight:500">${e.name}</span>${isL ? `<span class="badge bwa"><i class="ti ti-alert-triangle" style="font-size:10px"></i> Мало</span>` : ''}</div>
        <div style="display:flex;align-items:center;gap:8px"><div class="stk-bw"><div class="stk-bf" style="width:${p}%;background:${col}"></div></div><span style="font-size:11px;color:var(--color-text-tertiary)">Поставок: ${e.ship.length} · Итого: ${tot} л</span></div>
      </div>
      <div style="text-align:right;flex-shrink:0"><span style="font-size:20px;font-weight:700;color:${col}">${rem.toFixed(1)}</span> <span style="font-size:11px;color:var(--color-text-secondary)">л</span><div style="font-size:10px;color:var(--color-text-tertiary)">${p}%</div></div>
    </div>`;
  }).join('');
  const allH = mStock.flatMap(e => e.ship.map(s => ({ ...s, id: e.id, name: e.name }))).sort((a, b) => b.d.localeCompare(a.d));
  return `<div class="pad">
  <div class="ph"><h2>Склад СТМ</h2>${low > 0 ? `<span class="badge bwa"><i class="ti ti-alert-triangle" style="font-size:10px"></i> ${low} заканчивается</span>` : ''}</div>
  <div class="card"><div style="font-size:13px;font-weight:500;margin-bottom:.75rem">Текущие остатки</div>${rows || '<div style="font-size:13px;color:var(--color-text-tertiary);text-align:center;padding:1rem">Склад пуст</div>'}</div>
  <div class="card"><div style="font-size:13px;font-weight:500;margin-bottom:.625rem">История поставок</div>
    <table class="s"><colgroup><col style="width:15%"><col style="width:13%"><col style="width:33%"><col style="width:39%"></colgroup>
    <thead><tr><th>Дата</th><th>ID</th><th>Наименование</th><th>Объём</th></tr></thead>
    <tbody>${allH.map(h => `<tr><td style="color:var(--color-text-secondary)">${h.d}</td><td style="font-family:var(--font-mono);font-size:10px;color:var(--color-text-secondary)">${h.id}</td><td style="font-weight:500">${h.name}</td><td style="font-weight:500;color:#185FA5">+${h.v} л</td></tr>`).join('')}</tbody>
    </table>
  </div></div>`;
}
...
function pMWbl() {
  return `<div class="pad">
  <div class="ph"><h2>Накладные</h2></div>
  <div class="card"><div class="tw"><table class="s">
    <colgroup><col style="width:28%"><col style="width:13%"><col style="width:13%"><col style="width:20%"><col style="width:13%"><col style="width:13%"></colgroup>
    <thead><tr><th>№ транзакции</th><th>Дата</th><th>Устройство</th><th>Химия</th><th>Объём</th><th>Сумма</th></tr></thead>
    <tbody>${txns.filter(t => t.mer === MY_M).map(t => `<tr>
      <td style="font-family:var(--font-mono);font-size:10px">${t.id}</td>
      <td style="color:var(--color-text-secondary)">${t.date}</td>
      <td style="font-family:var(--font-mono)">${t.dev}</td>
      <td><div class="cp"><span class="cp-id">${t.c}</span><span>${CN[t.c]}</span></div></td>
      <td>${t.vol.toLocaleString('ru-KZ')} мл</td>
      <td style="font-weight:500">${t.amt.toLocaleString('ru-KZ')} ₸</td>
    </tr>`).join('')}</tbody>
  </table></div></div></div>`;
...
function openDE(id) {
  ...
  const chR = d.ch.map((ch, i) => {
    ...
    return `<div class="cel"><span style="font-size:11px;color:var(--color-text-secondary);font-weight:500">К${i + 1}</span><select id="cc${i}">${COPT.map(c => `<option value="${c}"${c === ch.c ? ' selected' : ''}>${c ? c + ' · ' + CN[c] : '— не назначен —'}</option>`).join('')}</select><label style="display:flex;align-items:center;gap:5px;font-size:11px;cursor:pointer"><input type="checkbox" id="ca${i}" ${ch.a ? 'checked' : ''} style="width:12px;height:12px"> Акт.</label></div><div style="padding:2px 0 4px;border-bottom:0.5px solid var(--color-border-tertiary)"><div style="display:flex;align-items:center;gap:7px;padding:0 0 0 54px"><div class="tbr2" style="width:100px"><div class="tbf2" style="width:${p}%;background:${ch.a && !e ? col : 'var(--color-border-tertiary)'}"></div></div><span style="font-size:10px;color:${ch.a && !e ? col : 'var(--color-text-tertiary)'}">${ch.l > 0 ? ch.l.toFixed(1) + ' л (авто)' : '—'}</span></div></div>`;
  }).join('');
  ...
  <div class="dvd"></div>
  <div class="cch"><span></span><span>Химия</span><span>Статус</span></div>
  ${chR}
  <div style="font-size:10px;color:var(--color-text-tertiary);display:flex;align-items:center;gap:4px;margin-top:4px"><i class="ti ti-info-circle" style="font-size:11px"></i>Бак ${CAP} л — константа.</div>
  <div class="dvd"></div>
...
function saveDE(id) { const d = devs.find(x => x.id === id); d.addr = document.getElementById('ea').value.trim() || d.addr; d.mer = document.getElementById('em').value; d.ad = document.getElementById('ead').value; d.note = document.getElementById('eco').value.trim(); for (let i = 0; i < 4; i++) { d.ch[i].c = document.getElementById('cc' + i).value; d.ch[i].a = document.getElementById('ca' + i).checked; } closeM(); render(); }

/* Merchant device edit */
function openME(id) { const d = devs.find(x => x.id === id); openM(`<div class="mh"><span class="mt2">Устройство — ${d.id}</span><button class="btn bg_" onclick="closeM()"><i class="ti ti-x"></i></button></div><div class="ff"><label>Комментарий</label><input type="text" id="mc" value="${d.note}"></div><div class="ib"><div class="ibl">Каналы химии (только просмотр)</div>${roRows(d)}<div class="ln"><i class="ti ti-lock" style="font-size:12px"></i>Управление химией — дистрибьютор</div></div><div class="mf"><button class="btn" onclick="closeM()">Отмена</button><button class="btn bp" onclick="devs.find(x=>x.id==='${id}').note=document.getElementById('mc').value.trim();closeM();render()"><i class="ti ti-check"></i> Сохранить</button></div>`); }

/* Merchant refill */
function openRef(id) {
  const d = devs.find(x => x.id === id);
  const ach = d.ch.filter(c => c.c && c.a);
  ...
  ${ach.length > 0 ? `<div style="font-size:11px;font-weight:500;color:var(--color-text-secondary);text-transform:uppercase;letter-spacing:.04em;margin-bottom:7px">Химия</div>${ach.map((c, i) => `<div class="ff"><label>${c.c} · ${CN[c.c] || ''} — остаток: <strong>${c.l.toFixed(1)} л</strong></label><input type="number" id="rc${i}" placeholder="Добавить л (макс. ${(CAP - c.l).toFixed(1)})" min="0" max="${(CAP - c.l).toFixed(1)}" step="0.1"></div>`).join('')}` : '<div style="font-size:12px;color:var(--color-text-tertiary);text-align:center;padding:7px">Нет активных каналов</div>'}
...
/* ── Freeze confirm ── */
function openFreezeConfirm(type, id) {
  ...
  <div class="fzb"><i class="ti ti-snowflake" style="font-size:16px;flex-shrink:0;margin-top:1px"></i><div><div style="font-weight:500;margin-bottom:2px">«${obj.name}»</div><div>Доступ в систему и платежи будут <strong>приостановлены</strong>. Устройства останутся активными.</div></div></div>
  <div class="mf"><button class="btn" onclick="closeM()">Отмена</button>
  <button class="btn bf" onclick="${type === 'dist' ? 'dists' : 'mercs'}.find(x=>x.id===${id}).st='frozen';closeM();render()"><i class="ti ti-snowflake" style="font-size:11px"></i> Подтвердить заморозку</button></div>`, true);
}
...
function openDistEdit(id) {
  ...
  <div style="display:flex;gap:7px;flex-wrap:wrap">
    ...
    ${isFrz ? `<button class="btn" style="color:#27500A;border-color:#A3D68A" onclick="dists.find(x=>x.id===${id}).st='paid';closeM();render()"><i class="ti ti-snowflake" style="font-size:12px"></i> Разморозить</button>` : `<button class="btn bf" onclick="openFreezeConfirm('dist',${id})"><i class="ti ti-snowflake" style="font-size:12px"></i> Заморозить</button>`}
...
function openEditMer(id) {
  ...
  <div style="display:flex;gap:7px;flex-wrap:wrap">
    ...
    ${isFrz ? `<button class="btn" style="color:#27500A;border-color:#A3D68A" onclick="mercs.find(x=>x.id===${id}).st='paid';closeM();render()"><i class="ti ti-snowflake" style="font-size:12px"></i> Разморозить</button>` : `<button class="btn bf" onclick="openFreezeConfirm('merch',${id})"><i class="ti ti-snowflake" style="font-size:12px"></i> Заморозить</button>`}
...
/* ── Photo grid (ассортимент / реклама) ── */
...
function openDelChem(id) { const c = chems.find(x => x.id === id); openM(`<div class="mh"><span class="mt2" style="color:#A32D2D">Удалить химию</span><button class="btn bg_" onclick="closeM()"><i class="ti ti-x"></i></button></div><div class="dw"><i class="ti ti-alert-triangle" style="font-size:17px;flex-shrink:0;margin-top:1px"></i><div><div style="font-weight:500;margin-bottom:2px">«${c.name}»</div><div>Химия будет удалена из ассортимента. Необратимо.</div></div></div><div class="mf"><button class="btn" onclick="closeM()">Отмена</button><button class="btn bds" onclick="chems=chems.filter(x=>x.id!=='${id}');closeM();render()"><i class="ti ti-trash" style="font-size:11px"></i> Удалить</button></div>`, true); }
...
function openDelAd(id) { const a = adsList.find(x => x.id === id); openM(`<div class="mh"><span class="mt2" style="color:#A32D2D">Удалить макет</span><button class="btn bg_" onclick="closeM()"><i class="ti ti-x"></i></button></div><div class="dw"><i class="ti ti-alert-triangle" style="font-size:17px;flex-shrink:0;margin-top:1px"></i><div><div style="font-weight:500;margin-bottom:2px">«${a.name}»</div><div>Макет будет удалён. Необратимо.</div></div></div><div class="mf"><button class="btn" onclick="closeM()">Отмена</button><button class="btn bds" onclick="adsList=adsList.filter(x=>x.id!=='${id}');closeM();render()"><i class="ti ti-trash" style="font-size:11px"></i> Удалить</button></div>`, true); }
